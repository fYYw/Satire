...
def main(args):
    ...
    dev_examples = mb.vec_minibatch(fake_dev + true_dev, word_dict, char_dict, args, False,
                                    char=True, sent_ling=True, doc_ling=False)
    ...
    dev_acc = evals.eval_vec_batch(eval_fn, dev_examples, char=True, sent_ling=True, doc_ling=False)
    ...
    for epoch in range(args.epoches):
        ...
        for batch_x, _ in train_examples:
            ...
            train_loss = train_fn(batch_rnn, batch_cnn, batch_word_mask,
                                  batch_sent_mask, batch_sent, batch_y)
            n_updates += 1
            if n_updates % 100 == 0 and epoch > 6:
                ...
                if dev_acc[3] > best_fsc and dev_acc[0] > best_acc:
                    ...
                    test_acc = evals.eval_batch(eval_fn, test_examples, word_dict, char_dict, args, doc_ling=False)
                    print('Performance on Test set: A: %.2f P:%.2f R:%.2f F:%.2f' % test_acc)
                    # util.save_params('char_sent_params_%.2f' % test_acc[3], params,
...
def create_theano_function(word_embed, char_embed, values=None):
    ...
    att_out, network_output, loss = fn.build_fn(word_x=word_x, char_x=char_x, word_mask=word_mask, sent_mask=sent_mask,
                                                ...
                                                args=args, sent_ling=sent_linguistic_x)
    ...
    train_fn = theano.function([word_x, char_x, word_mask, sent_mask, sent_linguistic_x, label_y],
                               loss, updates=updates)

    prediction = lasagne.layers.get_output(network_output, deterministic=True)
    eval_fn = theano.function([word_x, char_x, word_mask, sent_mask, sent_linguistic_x],
                              prediction)
