...
def main(args):
    ...
    fake_train, fake_dev, fake_test = du.load_fake(sent_ling=False)
    true_train, true_dev, true_test = du.load_true(sent_ling=False)
    if args.debug:
    ...
    dev_examples = mb.vec_minibatch(fake_dev + true_dev, word_dict, char_dict, args, False,
                                    char=True, sent_ling=False, doc_ling=True)
    ...
    dev_acc = evals.eval_vec_batch(eval_fn, dev_examples, char=True, sent_ling=False, doc_ling=True)
    ...
    for epoch in range(args.epoches):
        ...
        for batch_x, _ in train_examples:
            ...
            train_loss = train_fn(batch_rnn, batch_cnn, batch_word_mask,
                                  batch_sent_mask, batch_doc, batch_y)
            n_updates += 1
            if n_updates % 100 == 0 and epoch > 6:
                ...
                if dev_acc[3] > best_fsc and dev_acc[0] > best_acc:
                    ...
                    test_acc = evals.eval_batch(eval_fn, test_examples, word_dict, char_dict, args, sent_ling=False)
                    print('Performance on Test set: A: %.2f P:%.2f R:%.2f F:%.2f' % test_acc)
                    # util.save_params('char_doc_params_%.2f' % dev_acc[3], params,
