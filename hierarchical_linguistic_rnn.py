...
def main(args):
    ...
    dev_examples = mb.vec_minibatch(fake_dev + true_dev, word_dict, char_dict, args, shuffle=False, char=False)
    ...
    dev_acc = evals.eval_vec_batch(eval_fn, dev_examples, char=False)
    ...
    for epoch in range(args.epoches):
        ...
        if epoch > 3:
            ...
            att_fn, eval_fn, train_fn, params = \
                create_theano_function(word_embed, values=[x.get_value() for x in params])
        for batch_x, _ in train_examples:
            ...
            train_loss = train_fn(batch_rnn, batch_word_mask,
            ...
            if n_updates % 100 == 0 and epoch > 7:
                ...
                if dev_acc[3] > best_fsc and dev_acc[0] > best_acc:
                    ...
                    test_acc = evals.eval_batch(eval_fn, test_examples, word_dict, char_dict, args, char=False)
                    print('Performance on Test set: A: %.2f P:%.2f R:%.2f F:%.2f' % test_acc)
                    # util.save_params('char_hierarchical_rnn_params_%.2f' % dev_acc[3], params,
...
def create_theano_function(word_embed, values=None):
    ...
    att_out, network_output, loss = fn.build_fn(word_x=word_x, char_x=None, word_mask=word_mask, sent_mask=sent_mask,
                                                label_y=label_y, word_embed=word_embed, char_embed=None,
                                                args=args, doc_ling=doc_linguistic_x, sent_ling=sent_linguistic_x)
    ...
    train_fn = theano.function([word_x, word_mask, sent_mask, sent_linguistic_x, doc_linguistic_x, label_y],
                               loss, updates=updates)

    prediction = lasagne.layers.get_output(network_output, deterministic=True)
    eval_fn = theano.function([word_x, word_mask, sent_mask, sent_linguistic_x, doc_linguistic_x],
                              prediction)
    fn_check_attention = theano.function([word_x, word_mask, sent_mask, sent_linguistic_x],
                                         att_out)
